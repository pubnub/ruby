...
def prepare_message_actions_test(channel, messages_count, actions_count, recording = false)
  ...
  (1..messages_count).each { |index|
    publish_timetokens.push(@pubnub.publish(channel: channel, message: "Hello world #{index}", http_sync: true).timetoken.to_i)
    sleep rand(1..2) if recording
  }

  # Attach message actions to published messages.
  (1..actions_count).each { |index|
    action_actions.push(@pubnub.add_message_action(
      channel: channel,
